    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for lm in results.multi_hand_landmarks:
                ...
                # Prevent queue crashes
                if xy_queue.full():
                    try: xy_queue.get_nowait()
                    except: pass
                xy_queue.put((wrist_mm_x, wrist_mm_y))
...
def robot_control_process(xy_queue, client, robot_id, joint_indices):
    ...
    while True:
        try:
            wrist_mm_x, wrist_mm_y = xy_queue.get(timeout=0.1)
            ...
            # We use link 6 (the end-effector we added)
            ...
            func_grab(client, 1 if target_pos[2] < 0.20 else 0)
            
            print(f"✅ Hand({wrist_mm_x:.0f},{wrist_mm_y:.0f})mm → {target_pos}")
