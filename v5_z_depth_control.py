...
# Scaling for Z-movement (depth via wrist→MCP distance)
# 0.005 = small Z-range, 0.020 = larger Z-range
Z_SCALE = 0.010
...
WORKSPACE_Z        =  0.197  # meters — neutral grip height (at calibrated hand distance)
WORKSPACE_Z_MIN    =  -0.20   # meters — lowest allowed Z
WORKSPACE_Z_MAX    =  0.35   # meters — highest allowed Z
...
# Measured physical distance on your own hand between wrist (landmark 0) and
# index finger MCP (landmark 5) — re-measure with a ruler on your hand
# Larger value = arm is lower at neutral position, smaller = higher
HAND_WRIST_TO_MCP_MM = 80.0   # mm — adjust to your hand
...
def hand_tracking_process(xy_queue):
    ...
    while cap.isOpened() and not stop_event.is_set():
        ...
        # Stuur frame naar Unity camera stream
        if frame_queue.full():
...
def robot_control_process(xy_queue, client, robot_id, joint_indices):
    ...
    while True:
        try:
            ...
            # Convert naar degrees voor Unity
            q_deg = np.degrees(joint_pos[:6])
            
            # Send direct naar Unity
